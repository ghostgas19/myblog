...
import { Save, Loader2, Plus, Trash2, User, Heart, ImageIcon } from "lucide-react";
...
export default function AdminContentEditor({ initialProfile, initialRecommendations }: Props) {
  ...
  const addRec = () => {
    setRecs([...recs, { type: "Lagu", title: "", artist: "", emoji: "🎵" }]);
  };
  ...
  const updateRec = (index: number, field: keyof Recommendation, value: string) => {
    const newRecs = [...recs];
    ...
    <div className="space-y-12">
      ...
      <section className="bg-card border border-border rounded-sm p-6 overflow-hidden relative">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 className="font-serif text-xl font-bold">Kelola Profil</h2>
        </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="font-mono text-[10px] tracking-[2px] uppercase text-muted-foreground">Nama</label>
                <input
                  value={profile.name}
              ...
              <div className="space-y-2">
                <label className="font-mono text-[10px] tracking-[2px] uppercase text-muted-foreground">Role/Tagline</label>
                <input
                  value={profile.role}
            ...
            <div className="space-y-2">
              <label className="font-mono text-[10px] tracking-[2px] uppercase text-muted-foreground">Bio Singkat</label>
              <textarea
                value={profile.bio}
        ...
        <div className="mt-6 flex justify-end">
          ...
          >
            {isSavingProfile ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Save className="w-4 h-4" /> Simpan Profil</>}
          </button>
        </div>
      </section>
      ...
      <section className="bg-card border border-border rounded-sm p-6 relative">
        ...
        <div className="space-y-6">
          {recs.map((rec, i) => (
            <div key={i} className="flex flex-col md:flex-row gap-6 p-6 border border-border/50 bg-muted/30 rounded-sm group relative">
              ...
              <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="space-y-2">
                  <label className="font-mono text-[8px] tracking-[1px] text-muted-foreground uppercase">Tipe</label>
                  <select
                    value={rec.type}
                ...
                <div className="space-y-2">
                  <label className="font-mono text-[8px] tracking-[1px] text-muted-foreground uppercase">Judul</label>
                  <input
                    value={rec.title}
                ...
                <div className="space-y-2">
                  <label className="font-mono text-[8px] tracking-[1px] text-muted-foreground uppercase">Artis/Penulis</label>
        ...
        <div className="mt-8 flex justify-end pt-6 border-t border-border">
          <button
            ...
            className="flex items-center gap-2 bg-amber hover:bg-amber-light text-primary-foreground font-mono text-[10px] tracking-[2px] uppercase px-6 py-2.5 rounded-sm transition-all shadow-[2px_2px_0_theme(colors.maroon-deep)] active:shadow-none active:translate-x-0.5 active:translate-y-0.5"
          >
            {isSavingRecs ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Save className="w-4 h-4" /> Simpan Rekomendasi</>}
          </button>
        </div>
      </section>
    </div>
