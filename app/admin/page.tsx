    ...
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
      {[
        ...
        >
          ...
          <div>
            <div className="font-mono text-[10px] tracking-[2px] uppercase text-muted-foreground">
              {label}
            </div>
            <div className={`font-serif text-2xl font-bold ${color}`}>
              {value}
            </div>
          </div>
        </div>
        ...
        fallback={
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
            {Array.from({ length: 3 }).map((_, i) => (
              <div
                key={i}
                className="h-20 bg-card border border-border rounded-sm animate-pulse"
              />
            ))}
          </div>
        }
